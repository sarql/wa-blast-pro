    ...
    const processNextMessage = async () => {
      if (currentIndex < total) {
        ...
        if (!campaign.scheduledTime) {
          let currentDelay = 30;
          if (typeof campaign.delay === 'number') {
            currentDelay = campaign.delay;
          } else if (typeof campaign.delay === 'string') {
            if (campaign.delay === 'random_15_30') {
              currentDelay = Math.floor(Math.random() * (30 - 15 + 1)) + 15;
            } else if (campaign.delay === 'random_30_60') {
              currentDelay = Math.floor(Math.random() * (60 - 30 + 1)) + 30;
            } else {
              currentDelay = parseInt(campaign.delay, 10) || 30;
            }
          }

          setTimeRemaining(currentDelay);
